"""Tests for the mean-reversion signal template + executor factory."""
...
def test_mean_reversion_bottom_1_picks_lowest_lookback_return() -> None:
    ...
    # Slow ticker (MSFT) is the bottom-rank → mean-reversion picks it
    assert msft_signals.height == 3
    assert aapl_signals.height == 0
    msft_weights = msft_signals["weight"].unique().to_list()
    assert msft_weights == [1.0]


def test_mean_reversion_bottom_k_equals_universe_size_matches_buy_and_hold() -> None:
    """When bottom_k = N every ticker gets weight 1/N every day with
    enough history. Equivalent to buy-and-hold rebalanced daily — same
    behaviour as momentum_signals(top_k=N)."""
...
def test_mean_reversion_bottom_k_default_is_full_universe() -> None:
    """bottom_k=None defaults to len(plan.universe)."""
...
def test_mean_reversion_zero_weights_when_insufficient_history() -> None:
...
def test_mean_reversion_rejects_bottom_k_larger_than_universe() -> None:
    plan = _plan_one_fold(universe=("AAPL", "MSFT"))
    prices = _prices_long(
        [
            ("2025-06-01", "AAPL", 100.0),
...
def test_mean_reversion_rejects_nonpositive_lookback() -> None:
    plan = _plan_one_fold(universe=("AAPL",))
    prices = _prices_long([("2025-06-01", "AAPL", 100.0)])
    with pytest.raises(ValueError, match="lookback_days"):
        mean_reversion_signals(plan=plan, prices=prices, lookback_days=0)


def test_mean_reversion_signals_compose_with_run_walk_forward() -> None:
    """End-to-end: in a clearly trending environment, mean-reversion picks
    the laggard and earns less than momentum but the engine must still
    produce a sensible BacktestReport."""
    dates = [f"2025-06-{d:02d}" for d in range(1, 11)]
    ...
    fold = report.folds[0]
    # Engine must produce a valid report with a finite Sharpe and a
    # well-formed PnL number — value sign is strategy-dependent.
    assert isinstance(fold.pnl_usd, Decimal)


# === Factory: make_jquants_mean_reversion_executor ===


@pytest.mark.asyncio
@respx.mock
async def test_make_mean_reversion_executor_returns_protocol() -> None:
    async with JQuantsClient(api_key=_KEY) as client:
        executor: BacktestExecutor = make_jquants_mean_reversion_executor(
            client=client,
            lookback_days=5,
            bottom_k=1,
        )
...
@pytest.mark.asyncio
@respx.mock
async def test_make_mean_reversion_executor_pre_fetches_with_buffer() -> None:
    ...
    async with JQuantsClient(api_key=_KEY) as client:
        executor = make_jquants_mean_reversion_executor(
            client=client,
            lookback_days=21,
            bottom_k=1,
        )
...
@pytest.mark.asyncio
@respx.mock
async def test_make_mean_reversion_executor_runs_full_pipeline() -> None:
    """End-to-end: factory -> fetch prices -> mean-reversion signals
    -> run_walk_forward -> BacktestReport. Set up a universe where the
    mean-reversion ticker rebounds during the test window."""
    days_iso = [f"2025-05-{d:02d}" for d in range(1, 32)] + [
        f"2025-06-{d:02d}" for d in range(1, 11)
    ]
    # During lookback (May): MSFT trails AAPL strongly.
    # During test window (June): MSFT rallies fast (mean-reversion captures it).
    aapl_closes = []
    msft_closes = []
    for i in range(len(days_iso)):
        if i < 31:  # May: AAPL up, MSFT flat
            aapl_closes.append(100.0 + i * 1.0)
            msft_closes.append(200.0)
        else:  # June: MSFT rallies sharply
            june_idx = i - 31
            aapl_closes.append(130.0 - june_idx * 0.5)  # AAPL rolls over
            msft_closes.append(200.0 + june_idx * 3.0)  # MSFT strongly up
    ...
    fold = report.folds[0]
    # Mean-reversion picks MSFT (lagging through May), MSFT rallies in
    # June → positive PnL
